...
##################### initial SEIQ #################################
seiq_matrix = pd.read_csv('initial_seiq_omi_asym_0.5.csv')
seiq_matrix = seiq_matrix.iloc[:,1:]


##################### initial Susceptable #################################
suscept = np.array(pd.read_csv('./corona/corona_asym_0.5/suscept.csv'))[:,1:]
suscept_new = copy.deepcopy(suscept)


###############initial Exposed###############################
exposed =  np.array(pd.read_csv('./corona/corona_asym_0.5/exposed.csv'))[:,1:]


##############initial Infectious#############################
I_total_sym =  np.array(pd.read_csv('I_total_sym_omi_asym_0.5.csv'))
I_total_asym =  np.array(pd.read_csv('I_total_asym_omi_asym_0.5.csv'))

I_total = I_total_sym + 0.5*I_total_asym
I_total = np.array(pd.read_csv('./corona/corona_asym_0.5/I_total.csv'))[:,1:]
...
for r in range(88,1100):
    ...
    pd.DataFrame(q_list).to_csv('./corona/corona_asym_0.5/q_list.csv')
    pd.DataFrame(seiq_matrix[start:end+1,]).to_csv('./corona/corona_asym_0.5/seiq_matrix.csv')
    pd.DataFrame(suscept).to_csv('./corona/corona_asym_0.5/suscept.csv')
    pd.DataFrame(exposed).to_csv('./corona/corona_asym_0.5/exposed.csv')
    pd.DataFrame(I_total).to_csv('./corona/corona_asym_0.5/I_total.csv')
    k = k+1 
