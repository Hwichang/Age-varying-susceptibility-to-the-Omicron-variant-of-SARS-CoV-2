...
contact_home = np.array(pd.read_csv('contact_home.csv',header=None))
contact_others = np.array(pd.read_csv('contact_others.csv',header=None))
contact_all = 1.2*contact_school + contact_work + contact_home + contact_others

def contact_matrix(school_contact,S):
    res = 1.2*copy.deepcopy(school_contact)
...
##################### initial Susceptable #################################
suscept = np.array(pd.read_csv('suscept_omi.csv'))
...
for i in range(3):
    for j in range(q_standard[i],q_standard[i+1]):
        q_matrix[j,] = init_q[i,:]
...
for r in range(1100):
    ...
    pd.DataFrame(q_list).to_csv('./corona/corona_contact_0.8/q_list.csv')
    pd.DataFrame(seiq_matrix[start:end+1,]).to_csv('./corona/corona_contact_0.8/seiq_matrix.csv')
    pd.DataFrame(suscept).to_csv('./corona/corona_contact_0.8/suscept.csv')
    pd.DataFrame(exposed).to_csv('./corona/corona_contact_0.8/exposed.csv')
    pd.DataFrame(I_total).to_csv('./corona/corona_contact_0.8/I_total.csv')
