...
##################### initial Susceptable #################################
suscept = np.array(pd.read_csv('suscept_omi_lower_eff.csv'))
...
for r in range(1100):
    ...
    pd.DataFrame(q_list).to_csv('./corona/corona_lower_eff/q_list.csv')
    pd.DataFrame(seiq_matrix[start:end+1,]).to_csv('./corona/corona_lower_eff/seiq_matrix.csv')
    pd.DataFrame(suscept).to_csv('./corona/corona_lower_eff/suscept.csv')
    pd.DataFrame(exposed).to_csv('./corona/corona_lower_eff/exposed.csv')
    pd.DataFrame(I_total).to_csv('./corona/corona_lower_eff/I_total.csv')
